import React from 'react';
import { cn } from '@/lib/utils';

interface BackgroundDivProps {
  children: React.ReactNode;
  className?: string;
  eventSlug?: string;
  shouldCenter?: boolean;
  shouldImage?: boolean;
}

export function BackgroundDiv({ children, className, eventSlug, shouldCenter = true, shouldImage = false }: BackgroundDivProps) {
  const centerClass = shouldCenter ? 'flex items-center justify-center' : '';

  // Determine background style based on eventSlug and shouldImage combinations (memoized)
  const { backgroundStyle, backgroundClasses } = React.useMemo(() => {
    let style: React.CSSProperties = {};
    let classes = '';

    if (eventSlug && shouldImage) {
      // Event-specific background image
      style = {
        backgroundImage: `url(/${eventSlug}-background.webp)`,
        backgroundColor: 'hsla(var(--background), 0.6)'
      };
      // Vertical parallax by default (page scroll)
      classes = 'bg-center bg-parallax-vertical';
    } else if (eventSlug && !shouldImage) {
      // Event-specific color (using CSS custom properties for theming)
      style = {
        backgroundColor: 'hsl(var(--background))'
      };
    } else if (!eventSlug && shouldImage) {
      // Generic background image
      style = {
        backgroundImage: 'url(/bg.jpg)',
        backgroundColor: 'hsla(var(--background), 0.6)'
      };
      classes = 'bg-cover bg-center bg-no-repeat';
    } else {
      // Generic color (!eventSlug && !shouldImage)
      style = {
        backgroundColor: 'hsl(var(--background))'
      };
    }

    return { backgroundStyle: style, backgroundClasses: classes };
  }, [eventSlug, shouldImage]);

  return (
    <div
      className={cn(
        "min-h-screen w-full",
        backgroundClasses,
        centerClass,
        className
      )}
      style={backgroundStyle}
    >
      {children}
    </div>
  );
}